...
function financeCells(row, currency) {
  return [date(row.day, true), row.orders_created, row.orders_sent, money(row.estimated_revenue, currency), row.delivered, money(row.real_revenue, currency), money(row.costs?.product, currency), money(row.costs?.outbound_shipping, currency), money(row.costs?.cod, currency), money(row.costs?.outbound_fulfillment, currency), money(row.costs?.returns, currency), money(row.dropea_expenses, currency), money(row.dropea_profit, currency), financePercent(row.dropea_margin), money(row.costs?.advertising, currency), money(row.dropea_profit_after_meta, currency), money(row.costs?.fixed, currency), money(row.total_expenses, currency), money(row.operational_profit, currency), money(row.net_profit, currency), financePercent(row.roi), money(row.estimated_cpa, currency), money(row.real_cpa, currency), financePercent(row.confirmation_rate), financePercent(row.delivery_rate)];
}
function renderFinance() {
  ...
  $('finance-hero').replaceChildren(
    financeMetric('Beneficio Dropea', money(totals.dropea_profit, currency), `Ingresos entregados menos logística Dropea · margen ${financePercent(totals.dropea_margin)}`, totals.dropea_profit === null ? 'unknown' : Number(totals.dropea_profit) >= 0 ? 'positive' : 'negative'),
    financeMetric('Dropea − Meta', money(totals.dropea_profit_after_meta, currency), totals.dropea_profit_after_meta === null ? 'Pendiente de completar el gasto Meta diario' : `Publicidad: ${money(totals.costs?.advertising, currency)}`, totals.dropea_profit_after_meta === null ? 'unknown' : Number(totals.dropea_profit_after_meta) >= 0 ? 'primary' : 'negative'),
  ...
  const costs = [['Producto', totals.costs?.product], ['Envío de ida', totals.costs?.outbound_shipping], ['Fulfillment de ida', totals.costs?.outbound_fulfillment], ['Contra reembolso', totals.costs?.cod], ['Devoluciones', totals.costs?.returns], ['Publicidad', totals.costs?.advertising], ['Gastos fijos', totals.costs?.fixed]];
  ...
  const totalRow = { day: 'TOTAL', orders_created: totals.orders_created, orders_sent: totals.orders_sent, estimated_revenue: totals.estimated_revenue, delivered: totals.delivered, real_revenue: totals.real_revenue, costs: totals.costs, dropea_expenses: totals.dropea_expenses, dropea_profit: totals.dropea_profit, dropea_margin: totals.dropea_margin, dropea_profit_after_meta: totals.dropea_profit_after_meta, total_expenses: totals.total_expenses, operational_profit: totals.operational_profit, net_profit: totals.net_profit, roi: totals.roi, estimated_cpa: totals.orders_sent && totals.costs?.advertising !== null ? Number(totals.costs.advertising) / Number(totals.orders_sent) : null, real_cpa: totals.delivered && totals.costs?.advertising !== null ? Number(totals.costs.advertising) / Number(totals.delivered) : null, confirmation_rate: totals.confirmation_rate, delivery_rate: totals.delivery_rate };
...
$('logout-button').addEventListener('click', () => signOut(true)); $('refresh-button').addEventListener('click', refresh); $('finance-month').addEventListener('change', loadFinance); $('page-size').addEventListener('change', (event) => { state.limit = Number(event.target.value); state.offset = 0; loadQueue(); }); $('prev-page').addEventListener('click', () => { state.offset = Math.max(0, state.offset - state.limit); loadQueue(); }); $('next-page').addEventListener('click', () => { state.offset += state.limit; loadQueue(); }); document.querySelectorAll('.nav-item').forEach((item) => item.addEventListener('click', () => setView(item.dataset.view))); $('close-drawer').addEventListener('click', closeDetail); $('drawer-backdrop').addEventListener('click', closeDetail); document.addEventListener('keydown', (event) => { if (event.key === 'Escape') closeDetail(); });
