...
export async function executeObservedAddress(incident,expected,{env=process.env,readCurrent=readDropeaV2ReturnIssueState,readMessages=getIncidentChatMessages,claim=claimIncidentAddressResolution,finish=finishIncidentAddressResolution,write=provideDropeaV2AddressSolution,writeResolution=executeDropeaV2Resolution,prior=inspectPriorOrderReturn,claimMessage=claimTemplateDelivery,finishMessage=finishTemplateDelivery,send=sendTextMessage}={}){
 ...
 const d=addressResponseDecision({incident,messages,order:current.order,issue,previous:expected});
 ...
 if(d.action==='ASK_MISSING_FIELDS'){
   ...
   const currentDecision=fresh?addressResponseDecision({incident,messages:fresh,order:current.order,issue,previous:expected}):null;
 ...
 const action=d.action==='PROVIDE_ADDRESS_SOLUTION'?'PROVIDE_SOLUTION':d.action;
 const plan=d.provider_plan||planDropeaResolution(issue,action,{note:d.solution});
 if(!['PROVIDE_SOLUTION','CHANGE_ADDRESS','PICKUP_AT_AGENCY'].includes(action)||!plan.allowed)return {status:'BLOCKED_SOLUTION_CAPABILITY',verified:false};
 ...
 const last=finalMessages?addressResponseDecision({incident,messages:finalMessages,order:current.order,issue,previous:expected}):null;
