  ...
  if (process.argv.at(-1) === 'export') {
    ...
    for (const store of stores) {
      ...
      for (const order of result.items) {
        ...
        projection.push({ key:key(order.id),month:created.slice(0,7),outcome:returned?'RETURNED':delivery?'DELIVERED':null,
          settlementDay:returned || delivery,expense,final:b?.is_estimate===false });
      }
    }
