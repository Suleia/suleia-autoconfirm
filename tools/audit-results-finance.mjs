...
try {
  ...
  for (const month of months) {
    ...
    console.log(JSON.stringify({month, model:report.temporalModels, generatedAt:report.generatedAt,
      ...
      metaAudit:{panelSource:source?.sources?.meta,
        independentVerificationTool:'tools/audit-meta-finance-direct.mjs',
        legacyStoredComparison:{authoritativeForPanel:false,
          storedDays:[...metaByDay.keys()].filter(d=>d.startsWith(month)).length,
          ...
          lastSync:metaSource?.source?.lastSyncAt,
          limitation:'Legacy Supabase mirror is not the Meta Marketing API source used by the Results panel.'}},
