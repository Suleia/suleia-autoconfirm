...
const callback=(id='f1n100')=>({interactive:{button_reply:{id,title:'Mañana por la mañana'}}});
const now=new Date('2026-09-22T11:00:00Z');
test('only the exact ID in an observed contract becomes verified',()=>{
 assert.equal(verifyAbsentCallback(callback(),ABSENT_TEMPLATE_NAME,fixture(),now)?.canonical_payload,'ABSENT_TOMORROW_MORNING');
 assert.equal(verifyAbsentCallback(callback('ABSENT_TOMORROW_AM'),ABSENT_TEMPLATE_NAME,fixture(),now),null);
 assert.equal(verifyAbsentCallback(callback('f1n999'),ABSENT_TEMPLATE_NAME,fixture(),now),null);
});
test('configured, absent and future contracts cannot verify callbacks',()=>{
 for(const c of [null,{...fixture(),status:'CONFIGURED'},{...fixture(),observed_at:'2026-09-23T10:00:00Z'}])assert.equal(verifyAbsentCallback(callback(),ABSENT_TEMPLATE_NAME,c,now),null);
});
test('v2 context and duplicate provider IDs are forbidden',()=>{
 assert.equal(verifyAbsentCallback(callback(),'dropea_incidencia_ausente_v2',fixture(),now),null);
 const c=fixture();c.buttons[1].provider_id=c.buttons[0].provider_id;
 assert.equal(verifyAbsentCallback(callback(),ABSENT_TEMPLATE_NAME,c,now),null);
});
test('ambiguous IDs, label mismatch and missing observed notification block verification',()=>{
 assert.equal(verifyAbsentCallback({...callback(),button_payload:'f1n101'},ABSENT_TEMPLATE_NAME,fixture(),now),null);
 assert.equal(verifyAbsentCallback({...callback(),button_text:'Mañana por la tarde'},ABSENT_TEMPLATE_NAME,fixture(),now),null);
