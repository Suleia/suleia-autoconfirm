...
test('direct current reads remain authoritative when provider updated_at is newer than incremental row',async()=>{
 const x=fixture(),calls=[];x.issue.updated_at=x.issue.created_at;
 const runtime=createRecipientAbsentResolutionRuntime({pool:{query:async()=>({rows:[{...x.issue,identity_status:'EXACT',current:true}]})},projector:{},
  clients:[{store:{market:'ES',store_id:'store'},client:{request:async(name,params)=>{calls.push(name);assert.equal(params.id,name==='getIssue'?123:321);return {data:name==='getIssue'?{...x.issue,id:123,order_id:321,updated_at:new Date(Date.parse(x.issue.updated_at)+60000).toISOString()}:{id:321,store_id:'store',status:'SHIPPING',sub_status:'DELIVERY_EXCEPTION',customer_phone:'+34600000001'}};}}}],
