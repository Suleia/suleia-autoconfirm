...
export function absentResolutionControlVerified(control){
  return Boolean(['CANARY','LIVE'].includes(control?.status) && control.activation_at && Number.isFinite(Date.parse(control.activation_at))
...
export async function executeRecipientAbsentResolution({issueId,readFresh,readProviderIssue,writer,ledger,flags={},now=()=>new Date()}) {
  ...
  return ledger.withIssueLock(issueId,async store=>{
    ...
    catch {return {status:'UNVERIFIED',reason:'AUDIT_FINALIZATION_FAILED',writes:1,idempotency_key:resolution.idempotency_key};}
...
export function createAbsentResolutionLedger(pool){
  return {async withIssueLock(issueId,fn){
    ...
    try {
      ...
      const store={
        async control(){return (await client.query("SELECT status,activation_at,evidence FROM operations.recipient_absent_resolution_control WHERE workflow='RECIPIENT_ABSENT'")).rows[0] || null;},
        ...
        async claim(a){
          ...
          try {
          const control=(await client.query("SELECT status,activation_at,evidence,canary_issue_id FROM operations.recipient_absent_resolution_control WHERE workflow='RECIPIENT_ABSENT' FOR UPDATE")).rows[0];
