...
branch=feat/incident-autopilot
...
backup="/opt/suleia-backups/incident-autopilot-shadow-$revision"
...
image="suleia-incident-autopilot-shadow:$revision"
...
docker run --rm --network none --cap-drop ALL --security-opt no-new-privileges -e NODE_ENV=test \
  ...
  console.log("INCIDENT_AUTOPILOT_IMAGE_TESTS|PASS");' | tee "$backup/image-tests.txt"
...
rollback(){
  ...
  if [[ "${migration_applied:-false}" == true ]];then
    "${compose[@]}" exec -T postgres psql --no-psqlrc --set ON_ERROR_STOP=1 --username suleia_admin --dbname suleia_staging < migrations/rollback/040_incident_autopilot_shadow.down.sql || true
  fi
}
trap rollback ERR
"${compose[@]}" exec -T postgres psql --no-psqlrc --set ON_ERROR_STOP=1 \
  --username suleia_admin --dbname suleia_staging < migrations/040_incident_autopilot_shadow.sql
...
"${compose[@]}" exec -T --interactive=false postgres psql -X -At -U suleia_admin -d suleia_staging -c \
  "SELECT to_regclass('read_models.operations_incident_autopilot_current') IS NOT NULL" | grep -qx t
"${compose[@]}" exec -T --interactive=false postgres psql -X -At -U suleia_admin -d suleia_staging -c \
...
trap - ERR
printf 'INCIDENT_AUTOPILOT_DEPLOY|commit=%s|mode=SHADOW_READ_ONLY|env_preserved_except_read_budget=true|seven_other_services_unchanged=true|finance_renderer_preserved=true|migration=040|services=api,mcp,review,readonly-reader|business_actions=0|production_writes=0\n' "$revision"
