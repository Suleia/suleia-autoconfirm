...
SELECT 'url_audience_mapper=' || count(*) FROM protocol_mapper WHERE name = 'suleia-mcp-url-audience';
...
SELECT 'audience_custom_resource=' || count(*)
FROM protocol_mapper_config pmc
JOIN protocol_mapper pm ON pm.id = pmc.protocol_mapper_id
WHERE pm.name = 'suleia-mcp-url-audience'
  AND pmc.name = 'included.custom.audience'
  AND pmc.value = 'https://mcp.suleia.com/mcp';
...
grep -Fxq 'url_audience_mapper=1' <<<"${query_result}"
grep -Fxq 'audience_resource=1' <<<"${query_result}"
grep -Fxq 'resource_url=1' <<<"${query_result}"
grep -Fxq 'audience_custom_resource=1' <<<"${query_result}"
