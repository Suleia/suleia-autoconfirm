#!/usr/bin/env bash
# Isolated SHADOW-only deployment, preserving unrelated containers and secrets.
set -Eeuo pipefail
revision="${1:?exact commit required}"
[[ "$revision" =~ ^[0-9a-f]{40}$ ]] || exit 2
branch=fix/recipient-absent-shadow-integrity
...
backup="/opt/suleia-backups/absent-integrity-$revision"
...
for service in "${services[@]}" review-panel; do
...
image="suleia-absent-integrity:$revision"
...
jq -n --arg image "$image" --arg revision "$revision" --arg branch "$branch" --slurpfile cfg "$backup/current-compose.json" \
  ...
  def service($s;$name):{image:$image,environment:(($cfg[0].services[$name].environment|with_entries(.value=null))+($s[0][0].Config.Env|map(select(startswith("SULEIA_BUILD_")|not))|map(capture("^(?<key>[^=]+)=(?<value>.*)$"))|from_entries)+{"SULEIA_BUILD_REVISION":$revision,"SULEIA_BUILD_BRANCH":$branch,"AUSENTE_AUTOMATION_LIVE":"false","CHATBY_REAL_SENDS":"false","DROPEA_ACTIONS_ENABLED":"false","GLS_ACTIONS_ENABLED":"false"}),volumes:($s[0][0].Mounts|map({type:"bind",source:.Source,target:.Destination,read_only:true}))};
  {services:{api:service($api;"api"),"mcp-server":service($mcp;"mcp-server"),"ingestion-worker":service($worker;"ingestion-worker")}}' > "$override"
jq -n --slurpfile api "$backup/api-before.json" --slurpfile mcp "$backup/mcp-server-before.json" --slurpfile worker "$backup/ingestion-worker-before.json" --slurpfile panel "$backup/review-panel-before.json" '
...
rollback(){
  ...
  if [[ "$migrated" == true ]]; then "${compose[@]}" exec -T postgres psql -X -v ON_ERROR_STOP=1 -U suleia_admin -d suleia_staging < migrations/rollback/037_recipient_absent_shadow_integrity.down.sql; fi
}
trap rollback ERR
...
"${compose[@]}" exec -T postgres psql -X -v ON_ERROR_STOP=1 -U suleia_admin -d suleia_staging < migrations/039_absent_policy_registry_read_usage.sql
...
registry_hash="$(sha256sum migrations/039_absent_policy_registry_read_usage.sql | cut -d' ' -f1)"
"${compose[@]}" exec -T --interactive=false postgres psql -X -v ON_ERROR_STOP=1 -U suleia_admin -d suleia_staging \
  -c "INSERT INTO configuration.shadow_schema_releases(version,source_commit,migration_sha256) VALUES('039_absent_policy_registry_read_usage','$revision','$registry_hash') ON CONFLICT(version) DO NOTHING;"
...
for service in "${services[@]}"; do
  ...
  for phase in before after; do
    jq -c '.[0].Config.Env|map(select(test("^(SULEIA_BUILD_|AUSENTE_AUTOMATION_LIVE=|CHATBY_REAL_SENDS=|DROPEA_ACTIONS_ENABLED=|GLS_ACTIONS_ENABLED=)")|not))|sort' "$backup/$service-$phase.json" > "$backup/$service-$phase-env.json"
...
[[ "$healthy" == true ]]
sha256sum -c "$backup/env.sha256" >/dev/null
link="/opt/suleia-operations-absent-integrity-$revision"
[[ ! -e "$link" && ! -L "$link" ]] || exit 2
ln -s "$release" "$link";mv -T "$link" "$install"
trap - ERR
printf 'ABSENT_INTEGRITY_DEPLOY|commit=%s|env_preserved=true|other_services_unchanged=true|migration=037,038,039|services=api,mcp,ingestion,panel|mode=SHADOW\n' "$revision"
