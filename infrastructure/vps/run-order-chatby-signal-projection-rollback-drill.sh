...
BACKUP_FILE="${1:-}"
DRILL_DATABASE="suleia_order_chatby_signal_drill"
UP_MIGRATION="${INSTALL_ROOT}/migrations/018_order_chatby_signal_projection.sql"
DOWN_MIGRATION="${INSTALL_ROOT}/migrations/rollback/018_order_chatby_signal_projection.down.sql"
...
up_projection="$(compose exec --no-TTY postgres psql --no-psqlrc --tuples-only --no-align \
  --username suleia_admin --dbname "${DRILL_DATABASE}" --command \
  "select count(*) from pg_views where schemaname='read_models' and viewname='operations_order_context' and definition like '%operations_conversation_summaries%';")"
[[ "${up_projection}" = "1" ]]

compose exec --no-TTY postgres psql --no-psqlrc --set ON_ERROR_STOP=1 \
  --username suleia_admin --dbname "${DRILL_DATABASE}" < "${DOWN_MIGRATION}" >/dev/null
down_projection="$(compose exec --no-TTY postgres psql --no-psqlrc --tuples-only --no-align \
  --username suleia_admin --dbname "${DRILL_DATABASE}" --command \
  "select count(*) from pg_views where schemaname='read_models' and viewname='operations_order_context' and definition like '%operations_conversation_summaries%';")"
[[ "${down_projection}" = "0" ]]
...
cleanup
trap - EXIT
echo 'ORDER_CHATBY_SIGNAL_PROJECTION_ROLLBACK_DRILL|PASS|up_projection=1|down_projection=0|actions=0|production_writes=0'
