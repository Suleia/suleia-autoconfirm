...
branch=feat/autonomous-operations-ui
...
backup="/opt/suleia-backups/automation-ui-$revision"
...
image="suleia-automation-ui:$revision"
...
docker exec -i suleia-operations-staging-postgres-1 psql -X -v ON_ERROR_STOP=1 -U suleia_admin -d suleia_staging < migrations/047_operations_automation_read_models.sql
...
mv -T "$link" "$install"
trap - ERR
printf 'INCIDENT_PANEL_DEPLOY|commit=%s|env_preserved=true|nine_other_services_unchanged=true|migrations=1_readonly_views|services=api,panel|incident_external_writes=0\n' "$revision"
