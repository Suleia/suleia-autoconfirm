#!/usr/bin/env bash
# Incident presentation and read-only projection integrity. No migrations.
set -Eeuo pipefail
revision="${1:?exact revision required}"
[[ "$revision" =~ ^[0-9a-f]{40}$ ]] || exit 2
branch=feat/incident-panel-reference
...
backup="/opt/suleia-backups/incident-panel-reference-$revision"
...
mkdir -p "$backup"
chmod 0700 "$backup"
services=(api review-panel)
unchanged=(mcp-server ingestion-worker scheduler decision-engine postgres keycloak reverse-proxy mcp-edge monitoring)
...
# Provider automation, ingestion, incident projections and business decisions untouched.
git diff --exit-code "$(git -C "$resolved" rev-parse HEAD)" "$revision" -- autoconfirm services apps/api packages/suleia-operations-mcp migrations >/dev/null
image="suleia-incident-panel-reference:$revision"
...
jq -n --arg image "$image" --arg revision "$revision" --arg branch "$branch" --arg panel "$release/apps/review-panel" \
  --slurpfile cfg "$backup/current-compose.json" --slurpfile api "$backup/api-before.json" --slurpfile review "$backup/review-panel-before.json" '
  ...
  {services:{api:(original($api)+{image:$image,environment:(($cfg[0].services.api.environment|with_entries(.value=null))+(original($api).environment|with_entries(select(.key|startswith("SULEIA_BUILD_")|not)))+{"SULEIA_BUILD_REVISION":$revision,"SULEIA_BUILD_BRANCH":$branch})}),"review-panel":(original($review)|.volumes|=map(if .target=="/usr/share/nginx/html" then .source=$panel else . end))}}' > "$override"
jq -n --slurpfile api "$backup/api-before.json" --slurpfile review "$backup/review-panel-before.json" '
  def original($s):{image:$s[0][0].Config.Image,environment:($s[0][0].Config.Env|map(capture("^(?<key>[^=]+)=(?<value>.*)$"))|from_entries),volumes:($s[0][0].Mounts|map({type:"bind",source:.Source,target:.Destination,read_only:(.RW|not)}))};
  {services:{api:original($api),"review-panel":original($review)}}' > "$backup/rollback.json"
...
[[ "$healthy" == true ]]
for asset in index.html app.js results-dashboard.js results-dashboard.css incident-panel.js incident-panel.css;do
...
mv -T "$link" "$install"
trap - ERR
printf 'INCIDENT_PANEL_DEPLOY|commit=%s|env_preserved=true|nine_other_services_unchanged=true|migrations=0|services=api,panel|incident_external_writes=0\n' "$revision"
