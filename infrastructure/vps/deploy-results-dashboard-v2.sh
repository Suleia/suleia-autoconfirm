#!/usr/bin/env bash
# Results presentation and two additive read-only daily metrics. No migrations.
set -Eeuo pipefail
revision="${1:?exact revision required}"
[[ "$revision" =~ ^[0-9a-f]{40}$ ]] || exit 2
branch=feat/results-dashboard-v2
...
backup="/opt/suleia-backups/results-dashboard-v2-$revision"
...
image="suleia-results-dashboard-v2:$revision"
...
[[ "$healthy" == true ]]
for asset in index.html app.js results-dashboard.js results-dashboard.css;do
...
mv -T "$link" "$install"
trap - ERR
printf 'RESULTS_DEPLOY|commit=%s|env_preserved=true|nine_other_services_unchanged=true|migrations=0|services=api,panel|incident_external_writes=0\n' "$revision"
