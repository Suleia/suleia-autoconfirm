...
bash "${INSTALL_ROOT}/infrastructure/vps/run-platform-audit-readonly-rollback-drill.sh"
...
trap - ERR
rm -f "${ARCHIVE}"
echo "CHECKPOINT_H_DEPLOY|PASS|backup=verified|restore=verified|chatby_rollback=verified|data_model_rollback=verified|platform_role_rollback=verified|operations_rollback=verified|incident_rollback=verified|dropea_v2_rollback=verified|actions=0|production_writes=0"
