#!/usr/bin/env bash
# AUSENTE observability deployment; control flags and ingestion config are preserved.
set -Eeuo pipefail
revision="${1:?exact commit required}"
[[ "$revision" =~ ^[0-9a-f]{40}$ ]] || exit 2
branch=feat/absent-canary-observability
...
backup="/opt/suleia-backups/absent-observability-$revision"
...
git diff --exit-code "$(git -C "$resolved" rev-parse HEAD)" "$revision" -- autoconfirm apps/api packages/platform-core/src/finance apps/review-panel/results-dashboard.js apps/review-panel/results-dashboard.css apps/review-panel/styles.css infrastructure/docker ':(exclude)infrastructure/docker/Dockerfile.node' ':(exclude)apps/api/server.test.mjs' >/dev/null
...
[[ "$healthy" == true ]]
sha256sum -c "$backup/env.sha256" >/dev/null
link="/opt/suleia-operations-absent-observability-$revision"
...
printf 'ABSENT_OBSERVABILITY_DEPLOY|commit=%s|env_preserved=true|other_services_unchanged=true|migrations=NONE|services=api,mcp,ingestion,panel|writer=DISABLED|mode=SHADOW\n' "$revision"
