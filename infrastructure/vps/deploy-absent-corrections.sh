...
backup="/opt/suleia-backups/absent-corrections-$revision"
...
git diff --exit-code "$(git -C "$resolved" rev-parse HEAD)" "$revision" -- autoconfirm apps migrations packages/platform-core/src/finance infrastructure/docker >/dev/null
image="suleia-absent-corrections:$revision"
...
rollback(){
  trap - ERR
  "${compose[@]}" -f "$backup/rollback.json" up -d --no-deps --no-build api mcp-server ingestion-worker >/dev/null
}
trap rollback ERR
...
[[ "$healthy" == true ]]
sha256sum -c "$backup/env.sha256" >/dev/null
link="/opt/suleia-operations-absent-corrections-$revision"
[[ ! -e "$link" && ! -L "$link" ]] || exit 2
ln -s "$release" "$link";mv -T "$link" "$install"
trap - ERR
printf 'ABSENT_INTEGRITY_DEPLOY|commit=%s|env_preserved=true|other_services_unchanged=true|migrations=0|services=api,mcp,ingestion|mode=SHADOW\n' "$revision"
