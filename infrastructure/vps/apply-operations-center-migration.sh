...
MIGRATION_FILE="${INSTALL_ROOT}/migrations/006_operations_center_read_models.sql"

if [[ ! -r "${ENV_FILE}" || ! -r "${MIGRATION_FILE}" ]]; then
  echo "Operations Center migration inputs are missing." >&2
  exit 1
fi
...
echo "Operations Center read-model migration applied."
