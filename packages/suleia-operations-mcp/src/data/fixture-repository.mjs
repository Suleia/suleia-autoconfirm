...
export function createFixtureRepository(config, { anchor = new Date() } = {}) {
  ...
  return Object.freeze({
    ...
    async searchOrders({ status = null, limit = 50, offset = 0 } = {}) {
    ...
    async searchIncidents({ status = null, limit = 50, offset = 0 } = {}) {
    ...
    async searchOperationalFindings({ limit = 50, offset = 0 } = {}) {
      return { items: [], total: 0, limit, offset };
    },
    async getDatabaseSummary() {
      return { database: 'fixture', schema_count: 0, object_count: 0, read_model_count: 0,
        measured_at: new Date().toISOString() };
    },
    async getRuntimeMetrics() {
      return { database: 'fixture', database_size_bytes: 0, measured_at: new Date().toISOString() };
    },
    async getDatabaseCatalog({ platform = 'VPS_POSTGRES', limit = 50, offset = 0 } = {}) {
      return { platform, items: [], total: 0, limit, offset, metadata_only: true,
        arbitrary_sql_allowed: false, measured_at: new Date().toISOString() };
    },
