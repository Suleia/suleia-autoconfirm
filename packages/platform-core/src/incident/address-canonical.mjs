...
const sameTime=(a,b)=>Boolean(a&&b)&&Date.parse(a)===Date.parse(b);
...
export function materializeAddress({issue,observation:d={},policy,previous={},now=new Date()}) {
 ...
 const anchor=d.template_name==='dropea_incidencia_direccion_v1'&&d.notification_message_id
   &&Number.isFinite(Date.parse(d.notification_at))&&Date.parse(d.notification_at)>=Date.parse(issue.created_at)
 ...
 const blockers=[...(!verified?['CHATBY_CURRENT_READ_NOT_VERIFIED']:[]),...(!versionMatches?['OWNER_ISSUE_VERSION_MISMATCH']:[]),
