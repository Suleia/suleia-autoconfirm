import { absentHash, ABSENT_LIVE_FLAGS, ABSENT_TEMPLATE_NAME, ABSENT_FUTURE_RESPONSES } from './absent-template.mjs';
...
export function simulateRecipientAbsent(input, { now = new Date() } = {}) {
  ...
  const scoped = events.filter(e => e.canonical_issue_id === issue.canonical_issue_id && (!e.canonical_order_id || e.canonical_order_id === order.canonical_order_id)
  ...
  const response = latest ? interpretAbsentResponse(latest) : {customer_intent: chatbyCurrent ? 'NO_RESPONSE' : 'UNKNOWN', confidence: chatbyCurrent ? 1 : 0};
  ...
  const shadow = {decision_id:decisionId,policy_id:policy.policy_id || null,policy_version:'RECIPIENT_ABSENT_POLICY_V1',policy_snapshot_hash:policy.policy_snapshot_hash || ABSENT_POLICY_HASH,snapshot_status:policy.policy_id?'PERSISTED':'NOT_PERSISTED',
    ...
    prepared_response:response.button_pressed?ABSENT_FUTURE_RESPONSES[response.button_pressed]:null,prepared_response_action:response.button_pressed?'WOULD_SEND_RESPONSE':null,template_name:ABSENT_TEMPLATE_NAME,
