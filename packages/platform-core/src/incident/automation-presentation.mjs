...
export function autonomyMetrics(items){
 const unique=[...new Map(items.map(i=>[i.canonical_issue_id,i])).values()];
 ...
 return [metric('autonomous_resolution','Resolución autónoma',null,null,'Resoluciones VERIFIED sin intervención humana / resoluciones elegibles. Falta cobertura completa de intervención humana.'),metric('human_touch','Intervención humana',null,n,'Incidencias únicas con intervención humana / incidencias gestionadas. La cola de revisión no acredita intervención.'),metric('prepared','Acciones preparadas',unique.filter(i=>i.autonomy.status==='PREPARED').length,n,'Incidencias únicas preparadas / incidencias del periodo'),metric('human_review','Necesitan revisión',unique.filter(i=>i.autonomy.status==='HUMAN_REVIEW').length,n,'Incidencias únicas que requieren revisión / incidencias del periodo'),metric('execution_success','Éxito de ejecución',null,null,'Ejecuciones exitosas / ejecuciones reales intentadas; cobertura global no acreditada'),metric('verification_success','Verificación',null,null,'Acciones verificadas / acciones reales ejecutadas; cobertura global no acreditada'),metric('unknown','Ejecución no verificable',unique.filter(i=>i.execution.status==='UNKNOWN').length,n,'Incidencias con ejecución UNKNOWN / incidencias del periodo'),metric('customer_response','Respuesta del cliente',unique.filter(i=>i.recovery?.evidence?.valid_response).length,unique.filter(i=>i.recovery?.evidence?.notification_at).length,'Respuestas válidas / incidencias con aviso verificable'),metric('recovery','Recuperación',null,null,'Entregadas o recuperadas / intentos de recuperación; atribución canónica no disponible'),{id:'resolution_time',label:'Tiempo de resolución',value:null,definition:'Sin cobertura canónica de inicio y resolución atribuida'}];
}
