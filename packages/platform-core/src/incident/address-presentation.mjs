...
export function addressIncidentPresentation(item){
 ...
 const action=fresh&&!newer&&!uncertain&&(!canonical||item.notification_decision_current===true)?canonical?.action||d.action:'HUMAN_REVIEW',manual=['HUMAN_REVIEW','MANUAL_DISCOUNT_RECOVERY'].includes(action);
